...
## Introduction
This document explains the Firebase integration patterns used in the Property Tax Collector application. It covers SDK initialization, authentication flows (email/password), role-based access control, Firestore database operations for real-time synchronization, offline-first behavior via a service worker, and client-side photo handling including EXIF orientation detection and compression. It also outlines query strategies, data modeling decisions, and operational examples for user registration, property record creation, and worker management queries.
...
HTML["index.html<br/>Firebase SDKs, Auth, Firestore, Logic"]
...
- [index.html:1950-1967](file://index.html#L1950-L1967)
- [index.html:2218-2235](file://index.html#L2218-L2235)
- [sw.js:1-45](file://sw.js#L1-L45)

## Architecture Overview
The app initializes Firebase, authenticates users, and synchronizes data through Firestore. Real-time listeners keep worker dashboards and admin views up to date. A service worker caches assets and enables offline usage. Photos are processed client-side before being stored as data URLs.
...
- Records collection stores property data, ownership info, GPS, photo (data URL), and correction state.
...
- Photos are captured and processed client-side, then stored as data URLs within Firestore records.
- EXIF orientation is detected and applied to ensure proper display.
- Images are resized and compressed before stamping metadata and converting to JPEG.
- Bulk photo export is supported via ZIP generation.

Key implementation references:
- Photo capture and EXIF handling: [index.html:1838-1916](file://index.html#L1838-L1916)
- Orientation detection logic: [index.html:1752-1836](file://index.html#L1752-L1836)
- Photo compression and metadata stamp: [index.html:1881-1910](file://index.html#L1881-L1910)
- Photo export to ZIP: [index.html:2399-2432](file://index.html#L2399-L2432)

```mermaid
flowchart TD
Start(["Photo Selected"]) --> ReadEXIF["Read EXIF Orientation"]
ReadEXIF --> LoadImage["Load Image into Canvas"]
LoadImage --> Resize["Scale Down While Preserving Aspect"]
Resize --> Transform["Apply Orientation Transform"]
Transform --> Stamp["Stamp Metadata (ID, GPS, Timestamp, VC)"]
Stamp --> Compress["Convert to JPEG with Quality"]
Compress --> Store["Store as Data URL in Record"]
...
- Service worker caching: [sw.js:1-45](file://sw.js#L1-45)
...
- Property record creation: [index.html:1484-1623](file://index.html#L1484-L1623)
...
- Bulk data export: [index.html:2353-2520](file://index.html#L2353-L2520)
...
## Dependency Analysis
- The HTML file depends on CDN-hosted Firebase SDKs and the service worker for offline behavior.
...
## Conclusion
The Property Tax Collector integrates Firebase to deliver a robust, offline-first field data collection solution. Authentication and role-based access control secure the platform, while Firestore real-time listeners synchronize data across worker and admin views. Client-side photo processing streamlines uploads, and targeted queries ensure efficient performance. The service worker guarantees usability without connectivity, and the single-file architecture simplifies deployment and maintenance.